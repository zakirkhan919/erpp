...
class CommitteController extends Controller
{
    public function Committes()
    {
        ...
        return view('Finance::admin.committe.index', compact('members', 'districts'));
    }

    public function CommitteGet(Request $request)
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way.';
        }
        try {
            $list = Member::where('type_id', 2)->orderBy('id', 'desc')->get();
    ...
    public function CommitteSubmit(Request $request)
    {
        $member = Member::findOrFail($request->member_id);
        $member->committe_type = $request->committe_type;
        $member->type_id = 2;
        $member->save();
