...
class KholipaController extends Controller
{
    public function Kholipas()
    {
        $members = Member::all();
        $districts = District::all();
        return view('Finance::admin.kholipa.index', compact('members', 'districts'));
    }

    public function KholipaGet(Request $request)
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way.';
        }
        try {
            $list = Member::where('type_id', 3)->orderBy('id', 'desc')->get();
    ...
    public function KholipaSubmit(Request $request)
    {
